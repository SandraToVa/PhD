...
[s]=parameters3;
  ...
  if s==1 % CAS HYBRIT QUARKONIUM P^(+,0,-)
    if j==1
      r(1,1,i)=CC(x(i),j-1)-FF(x(i),j)+2*I1(j);
      r(2,2,i)=F(x(i),j)-GG(x(i),j)+2*E5(j);
      r(3,3,i)=A(x(i),j+1)-HH(x(i),j)+2*A9(j);
      r(4,4,i)=CC(x(i),j+1)+Vhf(x(i))+2*A1(j);

      r(1,2,i)=FG(x(i),j)+F4(j)+G2(j);
      r(2,1,i)=r(1,2,i);
      r(2,3,i)=GH(x(i),j)+B8(j)+D6(j);
      r(3,2,i)=r(2,1,i);
      r(3,4,i)=B(x(i),j+1)+A3(j)+A7(j);
      r(4,3,i)=r(3,4,i);
