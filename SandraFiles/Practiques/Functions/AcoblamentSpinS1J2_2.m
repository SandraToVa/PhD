...
[s]=parameters3;
  ...
  if s==1 % CAS HYBRIT QUARKONIUM P^(+,0,-)
    if j>=2 
      r(1,1,i)=F(x(i),j-1)+DD(x(i),j)+2*I5(j);
      r(2,2,i)=A(x(i),j)-DD(x(i),j)+2*E9(j);
      r(3,3,i)=CC(x(i),j)+BB(x(i),j)+2*E1(j);
      r(4,4,i)=F(x(i),j+1)-BB(x(i),j)+2*A5(j);
      
      r(1,2,i)=DE(x(i),j)+F8(j)+G6(j);
      r(2,1,i)=r(1,2,i);
      r(2,3,i)=B(x(i),j)+E3(j)+E7(j);
      r(3,2,i)=r(2,3,i);
      r(3,4,i)=BC(x(i),j)+B4(j)+D2(j);
      r(4,3,i)=r(3,4,i);
