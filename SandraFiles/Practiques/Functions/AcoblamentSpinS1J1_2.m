...
t=cputime;
[EigvData,meshData]=computeEigenvalues(system,0,10,3e-6);
...
[s]=parameters3;
  ...
  if s==1 % CAS HYBRIT QUARKONIUM P^(+,0,-)
    if j==1
      r(1,1,i)=A(x(i),j)-DD(x(i),j)+2*E9(j);
      r(2,2,i)=CC(x(i),j)+BB(x(i),j)+2*E1(j);
      r(3,3,i)=F(x(i),j+1)-BB(x(i),j)+2*A5(j);

      r(1,2,i)=B(x(i),j)+E3(j)+E7(j);
      r(2,1,i)=r(1,2,i);
      r(2,3,i)=BC(x(i),j)+B4(j)+D2(j);
      r(3,2,i)=r(2,3,i);
