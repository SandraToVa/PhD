...
t=cputime;
[EigvData,meshData]=computeEigenvalues(system,0,2,5e-9);
...
[s]=parameters3;
  ...
  if s==1 % CAS HYBRIT QUARKONIUM P^(+,0,-)
    if j>=2 
      r(1,1,i)=A(x(i),j-1)+Vhf(x(i))+2*I9(x(i),j);
      r(2,2,i)=CC(x(i),j-1)-FF(x(i),j)+2*I1(x(i),j);
      r(3,3,i)=F(x(i),j)-GG(x(i),j)+2*E5(x(i),j);
      r(4,4,i)=A(x(i),j+1)-HH(x(i),j)+2*A9(x(i),j);
      r(5,5,i)=CC(x(i),j+1)+Vhf(x(i))+2*A1(x(i),j);
      
      r(1,2,i)=B(x(i),j-1)+I3(x(i),j)+I7(x(i),j);
      r(2,1,i)=r(1,2,i);
      r(2,3,i)=FG(x(i),j)+F4(x(i),j)+G2(x(i),j);
      r(3,2,i)=r(2,3,i);
      r(3,4,i)=GH(x(i),j)+B8(x(i),j)+D6(x(i),j);
      r(4,3,i)=r(3,4,i);
      r(4,5,i)=B(x(i),j+1)+A3(x(i),j)+A7(x(i),j);
      r(5,4,i)=r(4,5,i);
