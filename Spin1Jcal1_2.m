...
t=cputime;
[EigvData,meshData]=computeEigenvalues(system,0,5,3e-9); 
...
[s]=parameters3;
  ...
  if s==1 % CAS HYBRIT QUARKONIUM P^(+,0,-)
    if j==1
      r(1,1,i)=A(x(i),j)-DD(x(i),j)+2*E9(x(i),j);
      r(2,2,i)=CC(x(i),j)+BB(x(i),j)+2*E1(x(i),j);
      r(3,3,i)=F(x(i),j+1)-BB(x(i),j)+2*A5(x(i),j);

      r(1,2,i)=B(x(i),j)+E3(x(i),j)+E7(x(i),j);
      r(2,1,i)=r(1,2,i);
      r(2,3,i)=BC(x(i),j)+B4(x(i),j)+D2(x(i),j);
      r(3,2,i)=r(2,3,i);
